            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {experienceItems.map((item) => (
                    <motion.div
                        ...
                        className={`glass-card p-8 h-[280px] flex flex-col group relative overflow-hidden cursor-pointer transition-colors shadow-lg ${
                            hw
                    ...
                    >
                        <motion.div
                            ...
                            className={`mb-8 w-14 h-14 flex items-center justify-center border shadow-inner transition-colors ${
                                hw
                        ...
                        <div className="fixed inset-0 z-[70] flex items-center justify-center p-6 sm:p-12 pointer-events-none">
                            {experienceItems.filter(item => item.slug === selectedId).map(item => (
                                <motion.div
                                    layoutId={`card-${item.slug}`}
                                    key={`modal-${item.slug}`}
                                    className={`glass-card p-8 sm:p-12 w-full max-w-3xl relative overflow-y-auto max-h-[90vh] pointer-events-auto ${
                                        hw
                                ...
                                >
                                    <button
                                        onClick={() => setSelectedId(null)}
                                        className={`absolute top-6 right-6 sm:top-8 sm:right-8 p-3 rounded-full transition-colors z-10 group ${
                                            hw
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-start gap-6 sm:gap-8 mb-8">
                                        <motion.div
                                            layoutId={`icon-${item.slug}`}
                                            className={`w-20 h-20 flex-shrink-0 flex items-center justify-center border shadow-2xl ${
                                                hw
